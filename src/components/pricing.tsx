...
export function Pricing() {
  ...
  return (
    <section id="precios" className="py-24 bg-[#fffbf9] relative overflow-hidden">
          ...
          {plans.map((plan, index) => (
            <div
              ...
              className={`relative rounded-3xl flex flex-col justify-between p-8 transition-all duration-300 ${
                plan.isPopular
                  ? "bg-white border-2 border-[#ff8b1e] shadow-2xl shadow-[#ff8b1e]/15 lg:-translate-y-2"
                  : "bg-white border border-[#f1dfd3] shadow-sm hover:border-[#ff8b1e]/50"
              }`}
            >
              {plan.isPopular && (
              ...
              <div>
                <div className="flex justify-between items-baseline mb-2">
                  <h3 className="text-xl font-bold text-[#171412]">{plan.name}</h3>
                </div>
                ...
                <div className="mb-6 pb-6 border-b border-[#f1dfd3]">
                  {typeof plan.priceMonthly === "number" ? (
                    <div className="flex items-baseline gap-1">
                      <span className="text-4xl sm:text-5xl font-extrabold text-[#171412]">
                        ${billingPeriod === "annual" ? plan.priceAnnual : plan.priceMonthly}
                      </span>
                      <span className="text-sm text-[#6b625b] font-medium">USD / mes</span>
                    </div>
                  ) : (
                    <div className="text-3xl sm:text-4xl font-extrabold text-[#171412]">
                      {plan.priceAnnual}
                    </div>
                  )}
              ...
              <div>
                <a
                  ...
                  className={`w-full py-3.5 px-4 rounded-xl font-semibold text-sm transition-all flex items-center justify-center gap-2 group ${
                    plan.isPopular
                      ? "bg-gradient-to-r from-[#ff8b1e] to-[#da551d] hover:brightness-105 text-white shadow-lg shadow-[#ff8b1e]/25 hover:scale-[1.02]"
                      : "bg-[#fff8f3] hover:bg-[#ffdfc5]/50 border border-[#f1dfd3] text-[#171412]"
                  }`}
                >
